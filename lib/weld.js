  ...
  window.weld = function(DOMTarget, data, pconfig) {
    ...
    ops = {
      siblings : function(parent, element, key, value) {
        ...
        currentSibling = siblings.length;
        element.weld = {
          parent  : parent,
        ...
        while (currentSibling--) {
          sibling = siblings[currentSibling];

          if (sibling === element) {
            // If this is not the last item in the list, store where new items should be inserted
            if (currentSibling < siblings.length) {
              element.weld.insertBefore = siblings[currentSibling+1];
            }
          ...
          // Check for the same class
            ...
            currentClass = classes.length;
            match        = true;
            while (currentClass--) {
              // TODO: optimize
              if (element.weld.classes.indexOf(classes[currentClass]) < 0) {
                match = false;
                break;
              }
            }
