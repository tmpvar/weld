  ...
  window.weld = function(DOMTarget, data, pconfig) {
    ...
    go  = function(where, parent, element, key, value) {
      ...
      if (ops[where]) {
        ...
        if (debug) {
          depth-=1;
          console.log(pad(), '└ ' + element + '' + (!!res ? color.green + ' ✓' + color.gray : ' ✗'));
        }
        return res;
      }
    ...
    ops = {
      traverse : function(parent, element, key, value) {
        ...
        // Object / Array-like
          ...
          if (array && templateParent) {
            templateSiblings = templateParent.children;
            siblingLength = templateSiblings.length;
            template.weld = {
              parent  : templateParent,
              classes : template.className.split(' ')
            };

            // Find all siblings that match the exact classes that exist in the originally
            // matched template node
            while (siblingLength--) {
              sibling = templateSiblings[siblingLength];

              if (sibling === template) {
                // If this is not the last item in the list, store where new items should be inserted
                if (siblingLength < templateSiblings.length) {
                  template.weld.insertBefore = templateSiblings[siblingLength+1];
                }

                // remove the template here because templateSiblings is a live list.
              ...
              // Check for the same class
                ...
                var classes      = sibling.className.split(' '),
                    totalClasses = classes.length,
                    match        = true;
                while (totalClasses--) {
                  // TODO: optimize
                  if (template.weld.classes.indexOf(classes[totalClasses]) < 0) {
                    match = false;
                    break;
                  }
                }
                ...
                if (match) {
                  templateParent.removeChild(sibling);
                }
              }
            }
      ...
      set : function(parent, element, key, value) {
        if(config.map && config.map(element, value, key) === false) {
          return false;
        }
