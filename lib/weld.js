...
;(function(window) {
  ...
  */
  select = function(key, ctx) {
    ...
    */
    ...
    weld.debug(0, "SELECTOR", selector, $(ctx).is(selector));
    ...
    weld.debug(0, 'SELECTING:', key, ret.length, "; matches:", !!ret);
    return ret;
  },
  ...
  */
  apply  = function(element, key, value) {
    ...
    var child = (element) ? select(key, element) : false
    
    // If there were no children found, return false.
    if (child === false) {// || child.__nonmatch) {
      weld.debug(-1, "CHILD NOT FOUND")
      return false;
    }
    ...
    // Handle IMG, set the src attribute.
      ...
      // For everything else, just set the text content.
        ...
        weld.debug(0, "RESULT:", child.tagName, $(child).attr('class'),'key:', key, 'value:', value)
        $(child).text(value); // To-Do: remove jQuery dependency
      }
    }
    weld.debug(-1, "/APPLY", child.outerHTML.replace(/[\r\n]*|  /g, ''));
    return true;
  },
  ...
  var weld = function(selector, data, pconfig) {
    ...
    */
    var walk = function(selected, key, data) {
      var result;
      weld.debug(1, "WALK", selected.length, key, data)
      selected = config.map ? config.map(selected, key, data) : selected;

      if (selected === false) {
        weld.debug(0, 'MAP MISS', key)
        return false;
      }
      ...
      // Array
        ...
        if (selected.length > 0) {
          for (i = 0; i<data.length; i++) {
            ...
            */
            weld.debug(0, "CLONING", selected[0].tagName, $(selected[0]).attr('class'));
            ...
            */
            ...
            if (!result) {
              weld.debug(0, 'WALK FAIL', key, result)
              continue;
            }
            ...
            */
            if (selected && selected.length > 0) {
              weld.debug(0, "INSERTING", el.outerHTML.replace(/[\r\n]+|  /g, ''), 'into', selected[0].tagName, selected[0].parentNode.className)
              insert(selected.parent(), el); // To-Do: remove jQuery dependency
            }
          }

          weld.debug(0, "REMOVING", $(remove).length, 'items');
          $(remove).each(function() {
            
            if (!this.__rendered || !config.overwrite) {
        ...
        for (var a in data) {
          if (data.hasOwnProperty(a)) {
            ...
            */
            ...
            var found = select(a, selected);
            weld.debug(0, "OBJECT:", a, 'found', !!found);
            if (!found) {
              found = selected;
            }
            walk(found, a, data[a]);
