;(function(window) {
   ...
   */
  window.weld = function weld(DOMTarget, data, pconfig) {
      ...
      traverse : function traverse(parent, element, key, value) {
        ...
        // ARRAY / NodeList
          ...
          for (i=0; i<l; i++) {
            ...
            if (!config.insert) {
              if (target.weld && target.weld.insertBefore) {
                templateParent.insertBefore(target, target.weld.insertBefore);
              } else {
                templateParent.appendChild(target);
              }
            } else {
              config.insert(templateParent, target);
            }
          }
