...
;(function(window) {
  ...
  */
  select = function(key, ctx) {
    ...
    */
    ...
    ctx = $(ctx);
    // Run the query
    ret = ctx.find(selector); // To-Do: remove jQuery dependency

    // If there are no results, use the incoming context as
    // the result.
    if (ret.length < 1 || ret[0] === ctx[0]) {
      ret = $(ctx); // To-Do: remove jQuery dependency
      ret.__nonmatch = true;
    }

    weld.debug('SELECTING:', key, ctx.length, "; matches:", !ret.__nonmatch);
    return ret;
  },
  ...
  */
  apply  = function(element, key, value) {
    ...
    var child = select(key, element).get(0); // To-Do: remove jQuery dependency
    weld.debug("SELECT RESULT", child.outerHTML);
    ...
    // Handle IMG, set the src attribute.
      ...
      // For everything else, just set the text content.
        ...
        weld.debug("APPLY:", child.tagName, $(child).attr('class'), key, value)
    ...
    */
    var walk = function(selected, key, data) {
      ...
      if (typeof data === "string") {
        ...
        return apply(selected, key, data);
      ...
      // Array
        ...
        if (selected.length > 0) {
          for (i = 0; i<data.length; i++) {
            ...
            */
            weld.debug("CLONING", selected[0].tagName, $(selected[0]).attr('class'));
            ...
            */
            weld.debug("INSERTING", el.outerHTML, 'into', selected[0].parentNode.tagName, $(selected[0]).attr('class'))
