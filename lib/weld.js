...
;(function(window) {
  var
  engine = window.$,
  jquery = engine.prototype.jquery,
  ...
  */
  select = function select(key, ctx) {
    ...
    */
    ...
    ret = engine(selector, ctx); // To-Do: remove jQuery dependency
  ...
  apply  = function apply(element, key, value) {
    ...
    // Handle IMG, set the src attribute.
      ...
      // For everything else, just set the text content.
        ...
        while (child.firstChild) { // Remove any existing nodes
  ...
  */
  ...
  if(jquery) {  
    engine.fn.weld = function(data, config) {
  ...
  var weld = function weld(selector, data, pconfig) {
    ...
    */
    selector = (typeof selector === "string") ?
                engine(selector)                   : // To-Do: remove jQuery dependency
                selector;
    ...
    var walk = function walk(selected, key, data) {
      ...
      // Array
        ...
        if (selected.length > 0) {
          ...
          engine(remove).each(function() { // To-Do: remove jQuery dependency
