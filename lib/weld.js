  ...
  */
  select = function(key, ctx) {
    ...
    */
    ...
    weld.debug(0, 'SELECTING:', key, ctx.length, "; matches:", !!ret);
    return ret;
  },
  ...
  apply  = function(element, key, value) {
    ...
    // Handle IMG, set the src attribute.
      ...
      // For everything else, just set the text content.
        ...
        weld.debug(1, "APPLY:", child.tagName, $(child).attr('class'),'key:', key, 'value:', value)
        $(child).text(value); // To-Do: remove jQuery dependency
      }
    }
    weld.debug(-1, "RESULT", child.outerHTML.replace(/[\r\n]*|  /g, ''));
    return true;
  },
    ...
    */
    var walk = function(selected, key, data) {
      ...
      if (typeof data === "string") {
        ...
        var res = apply(selected[0], key, data);
        weld.debug(-1, "/WALK", res)
        return;
        ...
        if (selected.length > 0) {
          for (i = 0; i<data.length; i++) {
            ...
            */
            ...
            if (!walk($(el), key, data[i])) {
              weld.debug(0, 'WALK FAIL', key)
              continue;
            }
